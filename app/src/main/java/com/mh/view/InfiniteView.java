...
public class InfiniteView extends BoardView {
     ...
     * @param level
     */
    public InfiniteView(Context context, int width,
                        int disruptLeftNum,
                        int tipLeftNum,
                        int hlepLeftNum,
                        int freezeLeftNum
...
//        drawAllIcons();
    ...
    public void freeze() {
        if (freezeLeftNum == 0) {
            ...
            pause();
            toolsChangeListener.onFreeze(freezeLeftNum, OnToolsChangeListener.TOOL_SUCCESS);            refreshHandler.sleep(3000, FREEZE);
